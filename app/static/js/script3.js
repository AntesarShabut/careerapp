...
async function storeSkillsAndRedirect() {
    const userSkills = {
      "A": rangeSlider.value,
    ...
    try {
      ...
      if (response.ok) {
          ...
          const encodedData = encodeURIComponent(JSON.stringify(data.session_data));
          window.location.href = '/matches_temp?session_data=' + encodedData;
      } else {
    ...
    //   // window.location.href = '/matches_temp?session_data=' + encodeURIComponent(storedData);
    // } else {
...
async function storeSkillsAndRedirect2() {
  const userSkills = [
      rangeSlider.value,
      rangeSlider2.value,
      rangeSlider3.value,
  ...
  let test = JSON.parse(sessionStorage.getItem('userSkills'));
  console.log(test[0]);

  const sessionData = sessionStorage.getItem('userSkills');

  const response = await fetch('/matches_temp', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
  ...
  window.location.href = '/matches_temp';
